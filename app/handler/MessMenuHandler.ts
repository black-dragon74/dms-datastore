...
async function MessMenuHandler(req: express.Request, res: express.Response) {
  ...
  if (!browserPath) {
    res
      .status(500)
      .send(errorToJSON("No exported variable `CHRPATH` found in env."));
  ...
  await page
    .goto(CONST.MESS_MENU_URL, { waitUntil: "networkidle2" })
    .catch((err) => {
      console.error(err);
      res.status(500).send(errorToJSON(err));
    });
  ...
  for (let meal of Object.keys(xPaths)) {
    if (meal == "last_updated_at") continue;
  ...
  // hack to flatten the last_updated_at element from the response
  if (resp["last_updated_at"].length > 0)
    resp["last_updated_at"] = resp["last_updated_at"][0];
